...
void Graph::buildEdge ()
{
    ...
    for (int from = 0; from < (int)size (); ++from)
        for (Vertex::edge_iterator it = (*this)[from].edge.begin (); it != (*this)[from].edge.end(); ++it)
            if(from < it->to) edges.push_back(*it);
        
    std::sort(edges.begin(), edges.end());
    
    for (Edge& edge:edges) {
        edge.id = id;
        std::sprintf(buf, "%d %d %d", edge.from, edge.to, edge.elabel);
        tmp[buf] = id;
        ++id;
    }
    for (int from = 0; from < (int)size (); ++from)
...
std::istream &Graph::read (std::istream &is)
{
	...
	while (true) {
		...
		unsigned int pos = (unsigned int)is.tellg ();
...
std::ostream &Graph::write (std::ostream &os)
{
	...
	for (int from = 0; from < (int)size (); ++from) {
		...
		for (Vertex::edge_iterator it = (*this)[from].edge.begin (); it != (*this)[from].edge.end (); ++it)
        {
            ...
            {
				std::sprintf (buf, "%d %d %d %.2f", from, it->to, it->elabel, it->probability);
			}
            else
            {
				std::sprintf (buf, "%d %d %d %.2f", it->to, from, it->elabel, it->probability);
			}
			tmp.insert (buf);
		}
	}
    ...
    void Graph::remove(int fromlabel, int elabel, int tolabel)
    {
        for (Graph::iterator from = this->begin(); from != this->end(); ++from) {
            for (Vertex::edge_iterator it = from->edge.begin();it != from->edge.end();) {
                int fl = from->label;
                int el = it->elabel;
                int tl = (*this)[it->to].label;
                if (fl > tl) _swap(fl, tl);
                if (fl == fromlabel && el == elabel && tl == tolabel)
                    it = (*from).edge.erase(it);
                else ++it;
            }
        }
    }
    
    std::ostream& Graph::write_pattern(std::ostream &os)
    {
        char buf[512];
        std::set<std::string> tmp;
        ...
        for (int from = 0; from < (int) size(); ++from)
        {
            ...
            for (Vertex::edge_iterator it = (*this)[from].edge.begin();
                 it != (*this)[from].edge.end(); ++it)
            {
                if (directed || from <= it->to) sprintf(buf, "%d %d %d", from, it->to, it->elabel);

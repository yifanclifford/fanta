...
Fanta::support_counts (Projected &projected)
{
...
//yifan
    void Fanta::run(unsigned int _pruning, bool _share, unsigned int _definition, unsigned int _maxpat, unsigned int _maxnode, double _epsilon, double _delta, double _sigma, unsigned int _phi, std::istream &_is, std::ostream &_os)
    {
        pruning = _pruning;
        share = _share;
        maxpat = _maxpat;
        ...
        printf("pruning\n");
        if(share) printf("share computation\n");
        //read
...
//                    edge_map[id] = p->edge->id;
        ...
        id = 0;
        for(std::map<int,int>::iterator entry = m.begin(); entry!=m.end(); ++entry){
//            printf("first=%d\n",i->first);
    ...
    void Fanta::edge_exact(Projected_map3 &root)
    {
        for (Projected_iterator3 fromlabel = root.begin(); fromlabel != root.end(); ++fromlabel)
            for (Projected_iterator2 elabel = fromlabel->second.begin(); elabel != fromlabel->second.end(); ++elabel)
                for (Projected_iterator1 tolabel = elabel->second.begin();tolabel != elabel->second.end();)
                {
                    ...
                    printf("(%d,%d,%d): sup=%d\n", fromlabel->first, elabel->first, tolabel->first, endpoint);
                    ++tolabel;
                }
    ...
    void Fanta::edge_expected(Projected_map3 &root)
    {
        for (Projected_iterator3 fromlabel = root.begin(); fromlabel != root.end(); ++fromlabel)
            for (Projected_iterator2 elabel = fromlabel->second.begin(); elabel != fromlabel->second.end(); ++elabel)
                for (Projected_iterator1 tolabel = elabel->second.begin();tolabel != elabel->second.end();)
                {
                    printf("edge (%d,%d,%d)",fromlabel->first, elabel->first, tolabel->first);
//                    index_watch.resume();
                    get_embeddings(tolabel->second);
//                    index_watch.pause();
                    endpoint = support_exact();
//                    printf(" endpoint=%d ",endpoint);
                    ...
                    std::vector<double>&pro = edge_pros[fromlabel->first][elabel->first][tolabel->first];
                    std::vector<double> spro;
                    for(int i=0;i<endpoint;++i) spro.push_back(1.0);
...
//                    *os << "edge ("<<fromlabel->first<<","<<elabel->first<<","<<tolabel->first<<"): ["<<lower<<","<<upper<<"]"<<std::endl;
        ...
        double error;
        for (std::map < int,std::map < int,std::map<int, std::vector<double> > > > ::iterator fl = edge_pros.begin(); fl != edge_pros.end(); ++fl)
            for (std::map < int, std::map<int, std::vector<double> > > ::iterator el = fl->second.begin(); el != fl->second.end(); ++el)
                for (std::map<int, std::vector<double> >::iterator tl = el->second.begin(); tl != el->second.end(); ++tl)
                {
                    error = sigma * epsilon / (2 * tl->second.size());
                    for (auto&p : tl->second)
                        if(p+error<1) p += error;
                        else p = 1;
                }
    }
...
//                    double& p = edge_pro[fromlabel->first][elabel->first][tolabel->first];
                    ...
                    printf("(%d,%d,%d) [%.2f,%.2f]\n", fromlabel->first, elabel->first, tolabel->first, lower, upper);
...
//                }
    ...
    void Fanta::project_exact(FANTA::Projected &projected)
    {
        ...
        // forward
        for (Projected_riterator3 from = forward.rbegin(); from != forward.rend(); ++from)
    ...
    void Fanta::project_expected(FANTA::Projected &projected, std::vector<double> &spro)
    {
//        printf("size=%d\n",maxpat);
        if(!DFS_CODE.check_label(maxnode) || DFS_CODE.size() > maxpat) return;
...
//            PRINT(pro, pro.size());
        ...
        {//the probabilities of basic edges are already calculated
    ...
    int Fanta::support_exact()
    {
        ...
        ProjectSet pset(len);
        for (auto& embedding : embeddings) pset += embedding.vertices;
        return pset.support();
    }
    ...
    double Fanta::compute_exact()
    {
        ...
        for(long long i=0;i<n;++i)
        {
            ...
            for (auto& embedding : embeddings)
            {
                ...
                if (contain) pset += embedding.vertices;
            }
    ...
    bool Fanta::support_expected(std::vector<double> &pro, std::vector<double> &spro)
    {
        int N, N1 = 0, n;
...
//        printf("t=%f\n",compute_watch.get_time());
        ...
        compute_watch.resume();
        if(share) p = share_probabilistic(N);
        else      p = compute_probabilistic(N);
        compute_watch.pause();
        lower = p - error;
        if(lower<0) lower = 0;
        upper = p + error;
        if(upper>1) upper = 1;
        if (upper < sigma) return false;
        return true;
    }
    ...
    int Fanta::evaluate_simple(std::vector<double> &pro,int x)
    {
        double sup = 0;
        if (x == endpoint) {
            for(int i=0;i<endpoint;++i) sup += pro[i];
            double error = epsilon*sigma/2;
            lower = sup - error;
            upper = sup + error;
            if(upper < sigma) return 2;
            else return 1;
        }
        return 0;
    }
    ...
    int Fanta::evaluate_check(std::vector<double> &pro, int x)
    {
        ...
        if(upper < sigma) return 2;
        if(lower > sigma*(1-epsilon)) return 1;
        return 0;
    }
    
    int Fanta::evaluate_prune(std::vector<double> &pro, std::vector<double> &spro, int x){
        lower = upper = 0;
        double error = epsilon * sigma / (2 * x);
        double a,b;
        for (int i=0; i<endpoint; ++i) {
            ...
            b = pro[i] + error;
            upper += MIN(a,b);
            if(pro[i]>error) lower+=pro[i]-error;
        }
        if(upper < sigma)  return 2;              //infrequent
        if(lower > sigma*(1-epsilon)) return 1;
        return 0;
    }
    ...
    {
        if(DFS_CODE.size() == 0) return false; //start from null
        int maxtoc = (int)embeddings[0].vertices.size();
        ...
        if (fromlabel0 > tolabel0) _swap(fromlabel0, tolabel0);
        bool I = true;
        for (int i = 0; i < DFS_CODE.size() - 1; i++) {
            ...
            if (fromlabel0 == fromlabel && e0.elabel == e.elabel && tolabel0 == tolabel)
            {
                I = false;
                break;
            }
...
//        printf("share time=%lld\n",share_time-old);
    ...
    void Fanta::share_dfs(int cur, int n, int &x, bool *edges, bool *projects, ProjectSet &pset) {
        int l = (int)edge_map.size();
        if(n==1 || cur==l){
            double r;
            ...
            for (int i=0; i<l; ++i) {
                ...
                contain[i] = contain1;
                if(contain1) ++cost2;
                if((contain1 && !contain2) || (!contain1 && contain2)) ++cost1;
            }
            if(cost1 < cost2) {
...
//            printf("left=%d,right=%d\n",left,n-left);
    ...
    int Fanta::enumerate_extensions(FANTA::Projected &projected, Projected_map2 &backward, Projected_map3 &forward)
    {
        ...
        for (unsigned int n = 0; n < projected.size(); ++n) {
            ...
            for (int i = (int)rmpath.size() - 1; i >= 1; --i)
            {
                ...
                if (e) backward[DFS_CODE[rmpath[i]].from][e->elabel].push(e, cur);
            }
            // pure forward
            if (get_forward_pure(G, history[rmpath[0]], minlabel, history, edges))
            {
                for (EdgeList::iterator it = edges.begin(); it != edges.end(); ++it)
                    forward[maxtoc][(*it)->elabel][G[(*it)->to].label].push(*it, cur);
            }
            // backtracked forward
            for (int i = 0; i < (int)rmpath.size(); ++i)
                if (get_forward_rmpath(G, history[rmpath[i]], minlabel, history, edges))
                    for (EdgeList::iterator it = edges.begin(); it != edges.end(); ++it)
                        forward[DFS_CODE[rmpath[i]].from][(*it)->elabel][G[(*it)->to].label].push(*it, cur);
        }
        return maxtoc;
    }
    ...
    void Fanta::run_exact()
    {
        ...
        for (unsigned int from = 0; from < G.size(); ++from)
            if (get_forward_root(G, G[from], edges))
                for (EdgeList::iterator it = edges.begin(); it != edges.end(); ++it)
                    root[G[from].label][(*it)->elabel][G[(*it)->to].label].push(*it, 0);

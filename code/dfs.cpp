...
DFSCode::nodeCount (void)
{
    ...
    bool DFSCode::check_label(int num){
        if(num>size()) return true;
        std::map<int,int> labelmap;
        ++labelmap[(*this)[0].fromlabel];
        for (unsigned int i=0; i<size(); ++i)
            if((*this)[i].from < (*this)[i].to)
                labelmap[(*this)[i].tolabel]++;
        for(std::map<int,int>::iterator label = labelmap.begin();label!=labelmap.end();++label)
            if(label->second > num){
//                printf("minlabel=%d",label->second);
                return false;
            }
        return true;
    }
